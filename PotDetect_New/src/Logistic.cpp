...
#include "PotDetect_New/Logistic.h"
	...
	while (iter < max_iter)
	{
		...
		for (int j = 0; j < featLen; ++j)
		{
			...
			for (int k = cur_index; k < cur_index + batch; ++k)
			{
				...
				if (method == 1)
				{
					sum += data[j] * batch_error[k - cur_index];
				}
				else if (method == 0)
				{
					sum += (-data[j]) * batch_error[k - cur_index] * batch_output[k - cur_index] * (1 - batch_output[k - cur_index]);
				}
				else if (method == 2)
				{
					sum += data[j] * batch_grad[k - cur_index];
				}
			}
...
float Logistic::predict(float *feat, int len)
{
	float sum = 0.0;
	for (int j = 0; j < len; ++j)
		sum += params[j] * feat[j];
...
int Logistic::classify(float *feat, int len)
{
	float prob = predict(feat, len);
	return prob > 0.5 ? 1 : -1;
}
